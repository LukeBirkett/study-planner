...
To evaluate these hypotheses, I implement a tiered experimental lineage which increases in complexity and acknowledgment of linguistic qualitys, i.e. words, structure, grammar, taggings. 

I start with a two-tiered set of of baselines. 

First, I conduct an unintelligent baseline by utilising a random (uniform) guessing approach. This gives me a lower bound to assess model efficacy against. 

I supplement this was an intelligent baseline through the use of a unigram bag of words classifer. This secondary baseline is really important because it largely encapsulates H1 as it is a pure vocabulary approach, meaning is there us substanial and (need a word here for extremely powerful) signal held within words taken in isolation then the later approaches will fail to build on this baseline in any meaningful way. Additionally, it provides the counterfactual for H2 in the sense that unigrams to do contain any signal on structure. 
