...
def aggressor(dt, inputs, params, state=[]) -> List[float]:
    ...
    left_speed_command = inputs[1] * params[0]
    # set right motor speed
    right_speed_command = inputs[0] * params[0]

    # return motor speeds to robot's controller
    return [left_speed_command, right_speed_command]

aggressor_controller = BraitenbergController(step_fun=aggressor, gain=4)
...
def coward(dt, inputs, params, state=[]) -> List[float]:
    ...
    # set right motor speed
    right_speed_command = inputs[1] * params[0]

    # return motor speeds to robot's controller
    return [left_speed_command, right_speed_command]

coward_controller = BraitenbergController(step_fun=coward, gain=4)

def lover(dt, inputs, params, state=[]) -> List[float]:

    # set left motor speed
    left_speed_command = 0.3 - (inputs[0] * params[0])
    # set right motor speed
    right_speed_command = 0.3 - (inputs[1] * params[0])
...
def monocular(dt, inputs, params, state=[]) -> List[float]:
    ...
    left_speed_command = inputs[0] * params[0]
    # set right motor speed
    right_speed_command = 1

    # return motor speeds to robot's controller
    return [left_speed_command, right_speed_command]

monocular_controller = BraitenbergController(step_fun=monocular, gain=10)
...
spike_noise_prob = 0.0 # try 0.025 at first
...
controller = monocular_controller
...
n_runs = 20 # number of simulations to run
