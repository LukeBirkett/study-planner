...
Network science provides a versatile framework for modeling complex systems, abstracting real-world interactions into formal graphs of nodes and links to evaluate overall topology and system dynamics. Its applications span identifying key central entities in social systems \cite{wasserman1994, newman2001}, detecting functional sub-communities \cite{fortunato2010}, modeling biological and information cascades \cite{pastor2001}, and assessing structural robustness against failures \cite{lusseau2003emergent, newman2003structure}. By shifting away from isolationist views of individual components, network science reveals the emergent organizational principles and collective behaviors governing modern complex systems.

\subsection{Why Apply Network Science to Football}
\label{subsec:why_network_football}
This report focuses on applying network science to sport \cite{araujo2006ecological}, specifically football \cite{duch2010quantifying}. Traditional football analytics relies primarily on isolated, terminal metrics such as completed passes, goals scored, or advanced modeled parameters like Expected Goals \cite{pollard1997xg} to quantify performance. However, evaluating players in isolation is fundamentally limited because football operates as a complex adaptive system \cite{buldu2019}. Team success cannot be understood through isolated actions alone, instead it emerges from continuous collective interactions, dynamic spatial relationships, and overarching tactical organization across the squad \cite{gama2026stoch}.

\subsection{How Apply Network Science to Football}
\label{subsec:how_network_football}
Applying network science requires decomposing a complex system into discrete nodes and directed edges. In football, outfield teammates are modeled as nodes ($N = 11$), while relational interactions between them form the connecting edges. Completed passes provide a pragmatic, objective event stream encoding tactical intent and spatial structure. Representing players as nodes and directed pass volumes as weighted edges forms the foundation of the PassMap paradigm \cite{buldu2018}. A comprehensive taxonomy of these network paradigms and their topological variations is detailed and visualized in Section 3.
% TODO: link section 3
...
The dataset comprises StatsBomb event data from the 2023/2024 FA Women's Super League (\texttt{competition\_id: 37}, \texttt{season\_id: 281}). Focusing on women's football directly addresses literature deficits regarding the overreliance on short men's samples and the systemic underrepresentation of longitudinal women's datasets \cite{alves2025review, gama2026stoch}.
...
\begin{figure}[htbp]
    ...
    \caption{Raw Pass Plotting: Sub-plot 1 Individual Player vs. Sub-plot 2 Whole Team.}
...
\begin{table}[htbp]
    \centering
    \caption{Highest Pass Sample Network Macro Degree Metrics}
...
\begin{figure}[htbp]
    ...
    \caption{Highest Pass Sample Network Betweenness Centrality $g(i)$ distribution.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/triad_plot.png}
    \caption{Top Transitive Triads Overlay.}
...
Benchmarking also requires controlling for tactical formation to normalize network topology. Segmenting even the largest pass-volume bin across the 11 recorded formations reduces the primary setup (4-2-3-1) to just 29 instances and 3 formation categories carry a single instance (\hyperref[tab:formation_binning]{Appendix Table~\ref{tab:formation_binning}}). Dual filtering for pass volume and formation dilutes sample sizes to near zero. Despite using a comprehensive season-wide dataset that is large by literature standards \cite{gama2026stoch}, these sparsity constraints necessitate generating spatial null models to evaluate extreme networks.
...
\subsection{Null Network Validation}
...
To distinguish genuine tactical adaptations from match-to-match noise, our generative resampling engine is parameterized across a large-scale league dataset comprising $N = 264$ match-team instances and $89,781$ completed passes. Constructing the first-order Markovian model directly from raw event logs---rather than finalized network topologies---ensures the resampling process inherently preserves spatial coordinates and physical game constraints.

Raw StatsBomb pitch coordinates ($120 \times 80$) are normalized to a standardized $100 \times 100$ scale and discretized into a $10 \times 10$ spatial grid of 100 uniform cells. Granular player positions are mapped into a condensed positional taxonomy (\hyperref[tab:condensed_positions_mapped]{Appendix Table~\ref{tab:condensed_positions_mapped}}), eliminating team-specific lateral biases while retaining essential functional roles. This spatial and positional aggregation maximizes data density per pitch cell, enabling the Markovian transition model to learn robust, role-based spatial transition probabilities across the dataset.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/pitch_grid.png}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{c X c c X X c}
        \toprule
        \textbf{Pass ID} & \textbf{Passer (Origin)} & \textbf{Bin} & \textbf{Drawn Pos.} & \textbf{Empirical} & \textbf{Resampled} & \textbf{Safeguard} \\
        \midrule
        101 & Wubben-Moy & (2, 4) & CB & Williamson & Williamson & Valid ($A \neq B$) \\
        102 & Little & (5, 5) & CM & Pelova & Pelova & Valid ($A \neq B$) \\
        103 & Catley & (7, 2) & LM & Foord & Foord & Valid ($A \neq B$) \\
        104 & Pelova & (5, 8) & RB & Fox & Fox & Valid ($A \neq B$) \\
        105 & Wubben-Moy & (4, 5) & CM & Little & Pelova & Valid ($A \neq B$) \\
        106 & Little & (8, 5) & ST & Russo & Blackstenius & Valid ($A \neq B$) \\
        \bottomrule
    \end{tabularx}
\end{table}
...
Visual comparison of the empirical network alongside a single spatial null realization demonstrates that spatial recipient resampling produces a natural, pitch-constrained topology. Unlike the Erd{\H{o}}s--R{\'e}nyi model, the rewired graph avoids pitch-wide ``hairball'' artifacts while maintaining realistic player positioning and passing channels.
...
\begin{table}[htbp]
    \centering
    \small
    \caption{Tier 1 Spatial Markovian Null Model Validation Summary ($N=500$ Iterations).}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{0.85\textwidth}{l X c c c c}
        \toprule
...
\begin{table}[htbp]
    \centering
    \small
    \caption{Null-Baselined Meso Evaluation Summary (Transitive Triads Top 20).}
    ...
    \begin{tabularx}{\textwidth}{c X c c c c}
        ...
        \midrule
        1  & Wubben-Moy -- Little -- Williamson & 116.00 & 79.23 & [61.00, 98.00] & $+3.83$ \\
        2  & Wubben-Moy -- Little -- Catley     & 111.00 & 61.11 & [44.00, 78.00] & $+5.67$ \\
        3  & Little -- Catley -- Pelova         & 76.00  & 56.74 & [39.00, 74.00] & $+2.11$ \\
        4  & Wubben-Moy -- Little -- Pelova     & 69.00  & 71.53 & [54.98, 89.00] & $-0.28$ \\
        5  & Little -- Williamson -- Pelova     & 67.00  & 66.89 & [50.00, 85.00] & $+0.01$ \\
        6  & Wubben-Moy -- Catley -- Pelova     & 65.00  & 49.48 & [34.00, 65.00] & $+1.97$ \\
        7  & Wubben-Moy -- Williamson -- Pelova & 60.00  & 57.81 & [42.00, 73.00] & $+0.28$ \\
        8  & Fox -- Williamson -- Pelova        & 55.00  & 37.90 & [24.00, 52.00] & $+2.47$ \\
        9  & Russo -- Little -- Pelova          & 51.00  & 36.34 & [21.00, 51.00] & $+1.87$ \\
        10 & Fox -- Little -- Pelova            & 51.00  & 40.84 & [25.00, 57.00] & $+1.30$ \\
        11 & Fox -- Little -- Williamson        & 51.00  & 38.20 & [23.00, 54.00] & $+1.63$ \\
        12 & Russo -- Little -- Williamson      & 42.00  & 24.05 & [10.00, 39.00] & $+2.44$ \\
        13 & Mead -- Williamson -- Pelova       & 40.00  & 25.52 & [15.00, 37.02] & $+2.44$ \\
        14 & Russo -- Wubben-Moy -- Little      & 39.00  & 25.58 & [12.00, 40.00] & $+1.84$ \\
        15 & Russo -- Williamson -- Pelova      & 38.00  & 22.44 & [9.00, 36.00]  & $+2.30$ \\
        16 & Russo -- Wubben-Moy -- Pelova      & 37.00  & 23.67 & [11.00, 37.00] & $+2.00$ \\
        17 & Russo -- Wubben-Moy -- Williamson  & 35.00  & 18.79 & [7.00, 30.00]  & $+2.78$ \\
        18 & Foord -- Little -- Catley          & 33.00  & 37.36 & [25.00, 50.00] & $-0.67$ \\
        19 & Foord -- Catley -- Pelova          & 33.00  & 32.95 & [21.00, 45.00] & $+0.01$ \\
        20 & Wubben-Moy -- Williamson -- D’Angelo & 33.00 & 32.70 & [21.00, 44.02] & $+0.05$ \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[htbp]
    \centering
    \small
    \caption{Null-Baselined Micro Evaluation Summary (Positional Betweenness Centrality).}
...
This research responds to direct calls in literature \cite{gama2026stoch} for robust null baselines. We validate that traditional randomizations, such as Erd{\H{o}}s--R{\'e}nyi, fail in this domain by destroying pitch geometry, flattening workload inequality, and generating structural role anomalies.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{l X}
        ...
        \addlinespace[3pt]
        \textbf{Node Transition Entropy} & \textbf{Individual Passing Variability:} Measures how evenly a player distributes passes among teammates. Captures diversity of passing choices rather than volume. High values indicate unpredictable distribution; low values suggest predictable passing patterns. \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{l X}
        ...
        \addlinespace[3pt]
        \textbf{Network Transition Entropy} & \textbf{Team-Level Passing Unpredictability:} Global measure of how evenly passes are distributed across all possible connections. Dense networks can still be predictable if passes concentrate on a few links; high entropy indicates overall tactical unpredictability for opponents. \\
        \bottomrule
    \end{tabularx}
\end{table}

% ------------------------------------------------------------------------------
% APPENDIX B: DATASET & POSITIONAL TAXONOMY
% ------------------------------------------------------------------------------
\clearpage
...
\begin{table}[htbp]
    \centering
    \small
    \caption{Distribution of Starting Formations Within the Modal Pass-Volume Tier (200--299 Passes, $n = 94$).}
    \label{tab:formation_binning}
    \begin{tabularx}{0.7\textwidth}{X c c}
        \toprule
        \textbf{Starting Formation} & \textbf{Match Count ($n$)} & \textbf{Percentage (\%)} \\
        \midrule
...
% ------------------------------------------------------------------------------
% APPENDIX C: NULL TRIAD EVALUATION
% ------------------------------------------------------------------------------
\clearpage
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{c X X X c}
        \toprule
        \textbf{Rank} & \textbf{Origin / Target ($A$)} & \textbf{Intermediate ($B$)} & \textbf{Target / Origin ($C$)} & \textbf{Bottleneck Capacity ($W_{\text{min}}$)} \\
        \midrule
...
% ------------------------------------------------------------------------------
% APPENDIX D: CONDENSED POSITIONAL TAXONOMY AND MAPPING LOGIC
% ------------------------------------------------------------------------------
\clearpage
...
\begin{table}[htbp]
    \centering
    \small
    \caption{Condensed Position Categories with Original Positions and Total Pass Volume.}
...
% ------------------------------------------------------------------------------
% APPENDIX E: ORIGINAL GRANULAR POSITION COUNTS
% ------------------------------------------------------------------------------
\clearpage
...
% ------------------------------------------------------------------------------
% APPENDIX F: EMPIRICAL VS. RESAMPLED PASS SHARE
% ------------------------------------------------------------------------------
\clearpage
...
% ==============================================================================
% APPENDIX G: SENSE CHECKING THE RESAMPLED RECIPIENT DATASET
% ==============================================================================
\clearpage
\begin{samepage}
...
\begin{figure}[H]
    ...
    \caption{Pitch plot illustrating spatial reception areas for the resampled striker position.}
...
% ------------------------------------------------------------------------------
% APPENDIX H: LEAGUE-WIDE STRIKER PASS SUMMARY
% ------------------------------------------------------------------------------
\clearpage
