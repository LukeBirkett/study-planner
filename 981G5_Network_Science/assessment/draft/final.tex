...
\begin{titlepage}
    ...
    % Course Title
    {\Large \textbf{Separating Noise from Architecture: A Spatial Markovian Null Model for Passing Networks in Women’s Football}}\\[0.3cm]
    {\large \textbf{Network Science (981G5)}}\\[0.1cm]
    
    % Author & Candidate Info
    \begin{tabular}{r l}
        \textbf{Author:} & Luke Birkett \\
        \textbf{Candidate ID:} & 22411525 \\
        \textbf{Word Count:} & ~4200* \\
        \textbf{Dataset Card:} & Yes (end of document) \\
    \end{tabular}\\[0.5cm]
    ...
    \begin{minipage}{0.92\textwidth}
        ...
        This research evaluates collective tactical behavior in football by integrating network science with spatiotemporal sports analytics. Using open source StatsBomb event data from the 2023/2024 FA Women's Super League, raw pass data is converted into directed, weighted PassMaps to address the underrepresentation of women's football in quantitative literature. We deployed a multiscale diagnostic suite across micro (betweenness centrality), meso (transitive triads), and macro (global shortest paths) graph dimensions to evaluate empirical match networks. Applying this framework to synthetic baselines demonstrated that traditional Erdos Renyi randomizations fail to produce valid football networks by destroying pitch geometry, flattening workload inequality, and generating structural role anomalies. In response, we engineered a novel first order Spatial Markovian generative null model. By shifting the analytical lens from dynamics \textit{on} the network to dynamics \textit{of} the network, the engine resamples pass recipients using league average spatial probabilities while preserving origin coordinates, passer identities, and pitch constraints. Executing this Monte Carlo pipeline against a high volume match successfully isolates true tactical signals, such as left flank overloads, from baseline geometric spatial noise.
    \end{minipage}\\[1cm]
    ...
    \begin{minipage}{0.92\textwidth}
        \tiny
        \color{gray}
        * \textit{Word Count Note:} The word count does not include contents pages, headings, tables, figures, or latex formulas. 
    \end{minipage}
...
\subsection{How to Apply Network Science to Football}
...
Evaluating unweighted degree across our high-volume sample network (\hyperref[tab:degree_metrics]{Table~\ref{tab:degree_metrics}}) yields a Mean Unweighted Degree of $\langle k \rangle = 16.18$, indicating high overall connectivity out of a theoretical maximum bound of 20. The low Coefficient of Variation ($CV_k = 0.1724$) and near-identical Normalized Second Moment ($\langle k^2 \rangle / \langle k \rangle = 16.66$) confirm that the existence of passing lanes is widely distributed across squad positions, eliminating absolute topological bottlenecks.
...
In our sample network (\hyperref[fig:betweenness_bar]{Figure~\ref{fig:betweenness_bar}}), central defenders Lotte Wubben-Moy ($g(i) = 0.3222$) and Leah Williamson ($g(i) = 0.3000$) record the highest centrality, acting as primary structural pivots during recycling and build-up phases---a finding consistent with contemporary football network literature \cite{alves2025review}.
...
This inversion addresses a critical research gap. Although stochastic flow models were originally used by football network researchers to bypass traditional null models, evaluating whether observed flow metrics reflect genuine tactical organization also ultimately requires benchmarking against a spatially constrained null ensemble \cite{gama2026stoch}.
...
\begin{table}[htbp]
    \centering
    \small
    \caption{Recipient Resampling Example and Player Mapping}
    \label{tab:sample_resampling_stream}
    \begin{tabularx}{\textwidth}{c X c c X X}
        \toprule
        \textbf{Pass ID} & \textbf{Passer} & \textbf{Bin} & \textbf{Resampled Pos} & \textbf{Empirical} & \textbf{Resampled} \\
        \midrule
...
Functional role realism is similarly preserved across all iterations. Goalkeeper Sabrina D’Angelo’s total pass volume stays tightly bound with an ensemble mean of $43.2300$ passes ($95\%$ CI: $[38.0000, 49.0000]$) and a range of $[35.0, 51.0]$. Unlike Erd{\H{o}}s--R{\'e}nyi random graphs that erroneously transform the goalkeeper into an active playmaking hub, the spatial engine encodes positional constraints across every generated instance.
...
\subsection{Macro-Level Evaluation: Global Average Shortest Path ($d$)}
\label{subsec:macro_evaluation}

Evaluating global average shortest path ($d = 0.1884$) against the 1000-iteration spatial null ensemble reveals that Arsenal WFC’s circulation efficiency emerges primarily from pass volume rather than unique tactics or elite performance (\hyperref[tab:null_macro_summary]{Table~\ref{tab:null_macro_summary}}).
...
\subsection{Meso-Level Evaluation: Transitive Triads ($I_{\text{transitive}}$)}
...
\subsection{Micro-Level Evaluation: Betweenness Centrality ($g(i)$)}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{c X X X c}
        \toprule
        \textbf{Rank} & \textbf{Origin ($A$)} & \textbf{Intermediate ($B$)} & \textbf{Target ($C$)} & \textbf{Capacity ($W_{\text{min}}$)} \\
        \midrule
...
% ------------------------------------------------------------------------------
\clearpage
\section{Condensed Positional Taxonomy and Mapping}
\label{app:cond_pos}
...
\clearpage
\section*{1. Dataset Overview \& FAIR Alignment}
\begin{itemize}
    \item \textbf{Dataset Name:} FA Women's Super League (WSL) Spatial PassMap and Markovian Null Ensembles (2023/2024).
    \item \textbf{Short Description:} A curated collection of directed, weighted passing networks and spatial probability tensors derived from the 2023/2024 FA Women's Super League season, alongside synthetic generative null networks.
    \item \textbf{Source:} Programmatically extracted via the \texttt{statsbombpy} API client accessing free StatsBomb open data.
    \item \textbf{Curator/Data Engineering:} Built using a custom Python pipeline (\texttt{pandas}, \texttt{networkx}, \texttt{matplotlib}, \texttt{scipy}) for the Network Science assessment.
    \item \textbf{License:} Open-access for academic research under the StatsBomb User Agreement.
    \item \textbf{FAIR Compliance:} Adheres to FAIR principles (Findable, Accessible, Interoperable, Reusable) by converting raw, proprietary spatiotemporal event streams into standardized adjacency matrices, percentage-scaled coordinates, and portable edge-list formats.
\end{itemize}

\section*{2. Motivation \& Purpose}
\begin{itemize}
    \item \textbf{Primary Purpose:} Created to lay the computational groundwork for domain-aware, spatially constrained generative null models for network analysis in association football.
    \item \textbf{Research Gap Addressed:} Directly responds to systemic underrepresentation of women's football in quantitative sports literature while addressing the lack of valid null baselines in spatial network science.
    \item \textbf{Intended Use Cases:} Multi-scale network diagnostics (micro-level betweenness centrality, meso-level transitive triad intensity, macro-level shortest path lengths) and benchmarking tactical passing structures.
    \item \textbf{Known Limitations:} Empirical benchmarking across real matches suffers from extreme data sparsity at high pass volumes ($>700$ passes). Grouping matches by formation and volume bins yields underpopulated sample sizes, highlighting the necessity of generative null modeling over pure empirical baselines.
\end{itemize}

\section*{3. Dataset Composition \& Network Topology}
\begin{itemize}
    \item \textbf{Network Format:} Directed, weighted, spatially constrained networks ($G = (V, E, W)$).
    \item \textbf{Nodes ($V$):} 11 nodes per network, representing the starting 11 tactical positions/active players with the highest appearance duration in each match-team instance.
    \item \textbf{Edges ($E$) \& Weights ($W$):} Directed pass counts ($A \to B$) representing successfully completed passes between teammates (incomplete or intercepted passes are filtered out).
    \item \textbf{Macro Corpus Scope:} 264 total team-match networks comprising 105,262 successful completed passes across the 2023/2024 WSL season, filtered to 89,781 for the scope of the project.
    \item \textbf{Case Study Network:} An extreme high-volume match (Arsenal WFC, Match ID 3913160; 847 total passes) utilized for stress-testing spatial null baselines against high-density graph topologies.
\end{itemize}

\section*{4. Data Collection, Normalization \& Engineering}
\begin{itemize}
    \item \textbf{API Ingestion:} Match lineups and event streams were fetched programmatically using \texttt{statsbombpy.sb.events()} and \texttt{statsbombpy.sb.lineups()}.
    \item \textbf{Positional Taxonomy Mapping:} Fine-grained StatsBomb positions (e.g., \textit{Left Center Back}, \textit{Right Center Back}) were mapped into a condensed 11-class taxonomy (e.g., \texttt{CB}, \texttt{CM}, \texttt{ST}) to maximize data density and eliminate team-specific lateral biases during probability modeling.
    \item \textbf{Spatial Normalization \& Binning:} Pitch coordinates were normalized from StatsBomb's raw scale ($120 \times 80$) to a standardized percentage scale ($100 \times 100$) and discretized into a $10 \times 10$ uniform spatial grid of 100 cells.
    \item \textbf{Markov Probability Tensor:} Compiled a season-wide frequency tensor of pass end locations binned by condensed recipient position. Applied Laplace additive smoothing ($\alpha = 1.0$) to establish a valid conditional probability model $P(\text{Position} \mid \text{Grid Bin})$.
    \item \textbf{Generative Recipient Resampling:} Synthetic null event streams were generated by preserving empirical origin coordinates, passer identities, and total pass counts while resampling recipient positions from $P(\text{Position} \mid \text{Grid Bin})$. Self-passes ($A \to A$) were strictly prevented via an iterative rejection sampler.
\end{itemize}

\section*{5. Ethical, Privacy \& Bias Considerations}
\begin{itemize}
    \item \textbf{Privacy \& PII:} Contains public-figure PII (player names, positions, and match appearance statistics). As all data is derived from publicly broadcast professional sports events and published open-access databases, it carries zero risk of disclosing sensitive personal, medical, or private private-life information.
    \item \textbf{Domain Equity:} Promotes open data accessibility in women's sports science by developing open-source visualization and graph analysis pipelines.
    \item \textbf{Tactical  Performance Biases:}
\end{itemize}

\section*{6. Suggested Analysis \& Benchmarking}
\begin{itemize}
    \item \textbf{Recommended Metrics:}
    \begin{itemize}
        \item \textit{Micro:} Unweighted degree ($k_{\text{in}}, k_{\text{out}}$), net flow ($\Delta s_i$), and weighted betweenness centrality $g(i)$.
        \item \textit{Meso:} Pure transitive triad intensity ($\bar{I}_{\text{transitive}}$) isolating progressive wall-pass combinations ($A \to B \to C$ with $A \to C$).
        \item \textit{Macro:} Global average shortest path length ($d_{\text{global}}$) using inverted weights ($l_{ij} = 1/w_{ij}$) and degree coefficient of variation ($CV_k$).
    \end{itemize}
    \item \textbf{Null Model Recommendations:} Users should benchmark empirical PassMaps against the provided 1st-order Spatial Markovian null model. Traditional randomizations (e.g., Erd{\H{o}}s--R{\'e}nyi $G(N,p)$ or degree-preserving rewiring) are fundamentally invalid for football networks because they destroy pitch geometry, flatten workload inequality, and generate impossible tactical anomalies (such as high-volume passing triangles between goalkeepers and strikers).
\end{itemize}
